...
public class VendingMachineAvatar : MonoBehaviour
{
    ...
    [SerializeField] private Vector2 dispenseOffset = new Vector2(0f, -0.5f);
    ...
    private IEnumerator PlaySequence(Sprite sprite, float lifetime, float scale)
    {
        ...
        GameObject drop = new GameObject("VendingDrop_" + sprite.name);
        drop.transform.SetParent(transform, false);
        Vector3 startPos = new Vector3(dispenseOffset.x, dispenseOffset.y, 0f);
    ...
    private IEnumerator Squash(Transform t)
    {
        ...
        Vector3 squashed = new Vector3(baseScale.x * (1f + landSquash), baseScale.y * (1f - landSquash), baseScale.z);
        float elapsed = 0f;
